    ...
    <>
      ...
      <div className="max-w-2xl space-y-3">
        <div className="flex items-center gap-3 rounded-lg border border-zinc-200 bg-white p-4">
          <Avatar name={user.name} seed={user.email} className="h-12 w-12 text-sm" />
          <div className="min-w-0">
            ...
            <p className="text-xs break-all text-zinc-500">{user.email}</p>
          </div>
        ...
        <dl className="divide-y divide-zinc-100 overflow-hidden rounded-lg border border-zinc-200 bg-white">
          {rows.map((row) => (
            <div
              key={row.label}
              className="flex flex-col gap-0.5 px-4 py-2.5 sm:flex-row sm:items-baseline sm:gap-4"
            >
              <dt className="shrink-0 text-xs text-zinc-500 sm:w-36">
                {row.label}
              </dt>
              <dd className="min-w-0 text-sm break-words text-zinc-900">
                {row.value}
              </dd>
            </div>
          ))}
